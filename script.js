...
async function loadData() {
    ...
    try {
        ...
        try {
            ...
            document.getElementById('updateTime').textContent = 
                '⚠️ 기본값 사용 중 (API 연결 실패)';
        } catch (fallbackError) {
            console.error('Fallback 업데이트도 실패:', fallbackError);
        }
    } finally {
...
async function fetchBTCPrice() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data.bitcoin && data.bitcoin.usd) {
                console.log('CoinGecko 성공:', data.bitcoin.usd);
                return {
        ...
        console.error('CoinGecko 실패:', error);
    }
    ...
    // Fallback: 합리적인 기본값
    console.warn('모든 BTC API 실패, 기본값 사용');
    return { 
        price: 95000, 
        change24h: 0 
    };
...
async function fetchMSTRPrice() {
    ...
    // 방법 1: Yahoo Finance
    try {
        const response = await fetch('https://query1.finance.yahoo.com/v8/finance/chart/MSTR?interval=1d&range=1d', {
            method: 'GET',
            headers: {
                'Accept': 'application/json'
            }
        ...
        console.error('Yahoo Finance 실패:', error);
    }
    
    // Fallback: 기본값
    console.warn('MSTR API 실패, 기본값 사용');
    return { 
        price: 350, 
        change24h: 0 
    };
